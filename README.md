...
This repository contains a complete deep learning pipeline for Multimodal Emotion Recognition using the [RAVDESS dataset](https://zenodo.org/records/1188976). It classifies 8 human emotions by processing:
1. **Audio**: Converted into Mel-Spectrograms and processed via a Convolutional Neural Network (CNN).
2. **Text**: Transcribed from audio using Hugging Face's Whisper model (`openai/whisper-tiny`), and processed via a Long Short-Term Memory (LSTM) network.
